...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      nav('/profile') // go to profile completion
    } catch (err: any) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, background: 'linear-gradient(160deg, #0d9488 0%, #0f766e 60%, #115e59 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem', position: 'relative', minHeight: '100vh' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <h2 style={{ color: 'white', fontSize: '1.8rem', fontWeight: 800, fontFamily: "'Space Grotesk', sans-serif", marginBottom: '0.75rem' }}>Join AI RunSense</h2>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.95rem', lineHeight: 1.6, maxWidth: '280px' }}>
            Start tracking your running form and unlock AI-powered coaching insights.
          </p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem', background: 'var(--bg-primary)' }}>
        <div style={{ maxWidth: '380px', width: '100%' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '0.5rem', color: 'var(--text-primary)' }}>Create Account</h1>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', fontSize: '0.9rem' }}>Join to track your running form over time.</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              ...
              <input className="input-field" type="text" placeholder="e.g. Sree" value={name} onChange={e => setName(e.target.value)} required />
            </div>
            <div>
            ...
            <button className="btn-primary" type="submit" style={{ marginTop: '0.5rem', width: '100%' }}>Sign Up</button>
          </form>

          <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            Already have an account? <button onClick={() => nav('/login')} style={{ background: 'none', border: 'none', color: 'var(--accent-teal)', cursor: 'pointer', fontWeight: 600 }}>Log in</button>
          </p>
        </div>
      </div>
    </div>
